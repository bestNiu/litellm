...
describe("LogDetailContent", () => {
  ...
  it("should display unionlabLLM Overhead when litellm_overhead_time_ms is in metadata", () => {
    render(
      <LogDetailContent
        logEntry={createLogEntry({
          metadata: {
    ...
    expect(screen.getByText("unionlabLLM Overhead")).toBeInTheDocument();
    expect(screen.getByText("42.50 ms")).toBeInTheDocument();
  });

  it("should not display unionlabLLM Overhead when litellm_overhead_time_ms is absent from metadata", () => {
    render(<LogDetailContent logEntry={createLogEntry({ metadata: { status: "success" } })} />);

    expect(screen.queryByText("unionlabLLM Overhead")).not.toBeInTheDocument();
  });
